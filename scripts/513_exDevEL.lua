...
local complexButton = luajava.new(ComplexButton, "", AssetManager:getLabelStyle(24), luajava.createProxy(RunnableInterface, {
    run = function()
        UiManager.dialog:showAlert("Not implemented")
    end
}))
...
complexButton:setIcon(AssetManager:getDrawable(exdevel.devmode and "icon-floppy" or "exdevel-logo"), icon_pos.x, icon_pos.y, 32, 32)
...
toggleButtonLayer:getTable():add(complexButton):top():left():size(size.x, size.y):padTop(132):padLeft(80)
